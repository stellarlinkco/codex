#!/usr/bin/env python3
"""Harness Stop hook — blocks Codex from stopping while work remains.

Strict by default: if eligible tasks remain, it blocks Stop indefinitely.
Optionally, set `HARNESS_STOP_TIMEOUT_SECONDS` to allow Stop after a wall-clock
timeout without progress, to prevent infinite loops.
"""
...
# Optional safety valve to prevent infinite stop-hook loops.
# If unset/empty, the hook never allows stop early (strict).
...
def _read_block_counter(root: Path) -> tuple[int, int, Optional[float]]:
    """Returns (consecutive_blocks, last_completed_count, first_block_unix_ts)."""
    p = _block_counter_path(root)
    try:
        raw = p.read_text("utf-8").strip()
        parts = raw.split(",")
        blocks = int(parts[0]) if len(parts) > 0 else 0
        completed = int(parts[1]) if len(parts) > 1 else 0
        first_ts = float(parts[2]) if len(parts) > 2 and parts[2].strip() else None
        return blocks, completed, first_ts
    except Exception:
        return 0, 0, None


def _write_block_counter(root: Path, blocks: int, completed: int, first_ts: Optional[float]) -> None:
    p = _block_counter_path(root)
    tmp = p.with_name(f"{p.name}.tmp.{os.getpid()}")
    try:
        ts = "" if first_ts is None else str(first_ts)
        tmp.write_text(f"{blocks},{completed},{ts}", encoding="utf-8")
        os.replace(tmp, p)
    except Exception:
        try:
...
def _stop_timeout_seconds() -> Optional[float]:
    raw = os.environ.get("HARNESS_STOP_TIMEOUT_SECONDS")
    if raw is None:
    ...
    try:
        ...
        return DEFAULT_STOP_TIMEOUT_SECONDS
    return v if v > 0 else DEFAULT_STOP_TIMEOUT_SECONDS
...
def main() -> int:
    ...
    if max_sessions > 0 and session_count >= max_sessions:
        # In strict mode we do not allow stop purely due to a session limit; rely on
        # max_attempts + optional timeout instead.
        pass
    ...
    if not is_concurrent and max_tasks_per_session > 0 and session_count > 0 and progress_path.is_file():
        ...
        if finished >= max_tasks_per_session:
            # In strict mode we do not allow stop purely due to a per-session limit; rely on
            # max_attempts + optional timeout instead.
            pass
    ...
    # Optional safety valve: allow stop after a wall-clock timeout without progress.
    # Default is strict (no timeout) so stop remains blocked until all tasks reach a terminal state.
    timeout_s = _stop_timeout_seconds()
    now = float(payload.get("now_unix_ts") or 0) or None
    if now is None:
        try:
    ...
    prev_blocks, prev_completed, prev_first_ts = _read_block_counter(root)
    if completed_count > prev_completed:
        # Progress was made, reset counter
        prev_blocks = 0
        prev_first_ts = None
    consecutive = prev_blocks + 1
    first_ts = prev_first_ts if prev_first_ts is not None else (now if now else None)
    _write_block_counter(root, consecutive, completed_count, first_ts)

    if stop_hook_active and timeout_s is not None and first_ts is not None and now:
        elapsed = now - first_ts
        if elapsed >= timeout_s:
            _reset_block_counter(root)
            sys.stderr.write(
                "HARNESS: WARN — Stop hook timeout exceeded without progress. Allowing stop.\n"
            )
            return 0
