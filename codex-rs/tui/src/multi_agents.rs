...
pub(crate) fn sort_agent_picker_threads(agent_threads: &mut [(ThreadId, AgentPickerThreadEntry)]) {
    agent_threads.sort_by(|(left_thread_id, left), (right_thread_id, right)| {
        left.is_closed
            .cmp(&right.is_closed)
            .then_with(|| left.agent_nickname.cmp(&right.agent_nickname))
