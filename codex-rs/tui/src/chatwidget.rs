...
impl ChatWidget {
    ...
    pub(crate) fn restore_terminal_title(&mut self, items: Option<Vec<String>>) {
        self.config.tui_terminal_title = items;
    }

    pub(crate) fn set_approvals_reviewer(
        &mut self,
        policy: codex_core::config::types::ApprovalsReviewer,
    ) {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let params = crate::bottom_pane::feedback_upload_consent_params(
            ...
            self.current_rollout_path.clone(),
            snapshot
                .feedback_diagnostics_attachment_text(/*include_logs*/ true)
                .is_some(),
        );
    ...
    fn on_guardian_assessment(&mut self, ev: codex_protocol::approvals::GuardianAssessmentEvent) {
        let status = match ev.status {
            codex_protocol::approvals::GuardianAssessmentStatus::InProgress => {
                self.bottom_pane.ensure_status_indicator();
                self.bottom_pane
                    .set_interrupt_hint_visible(/*visible*/ true);
                self.set_status_header("Guardian review in progress".to_string());
                return;
            }
        ...
        let mut message = format!("Guardian review {status}");
        if let Some(risk_level) = ev.risk_level {
            message.push_str(&format!(" ({risk_level:?})"));
        }
        let hint = ev.rationale.filter(|rationale| !rationale.is_empty());
        if matches!(
            ev.status,
    ...
    fn on_hook_started(&mut self, event: codex_protocol::protocol::HookStartedEvent) {
        let label = match event.run.event_name {
    ...
    fn on_hook_completed(&mut self, event: codex_protocol::protocol::HookCompletedEvent) {
        ...
        let mut lines: Vec<Line<'static>> = vec![format!("{label} hook ({status})").into()];
    ...
    pub(crate) fn handle_exec_approval_now(&mut self, ev: ExecApprovalRequestEvent) {
        ...
        self.bottom_pane
            .push_approval_request(request, self.config.features.get());
        self.request_redraw();
    }
    ...
    pub(crate) fn handle_request_permissions_now(&mut self, ev: RequestPermissionsEvent) {
        ...
        let request = ApprovalRequest::Permissions {
            ...
            call_id: ev.call_id,
            reason: ev.reason,
            permissions: ev.permissions.into(),
        };
        self.bottom_pane
            .push_approval_request(request, self.config.features.get());
        self.request_redraw();
    ...
    fn configured_terminal_title_items(&self) -> Vec<String> {
        self.config.tui_terminal_title.clone().unwrap_or_default()
    }
    ...
    pub(crate) fn add_plugins_output(&mut self) {
        self.add_info_message(
            "Plugins browser is not wired into this TUI surface yet.".to_string(),
            /*hint*/ None,
        );
    }
    ...
    pub(crate) fn open_plugin_detail_loading_popup(&mut self, plugin_display_name: &str) {
        self.add_info_message(
            format!("Loading plugin details for {plugin_display_name}..."),
            /*hint*/ None,
        );
    }
    ...
    pub(crate) fn on_plugins_loaded(
        &mut self,
        _cwd: PathBuf,
        result: Result<codex_app_server_protocol::PluginListResponse, String>,
    ) {
        if let Err(err) = result {
            self.add_error_message(format!("Failed to load plugins: {err}"));
        }
    }
    ...
    pub(crate) fn on_plugin_detail_loaded(
        &mut self,
        _cwd: PathBuf,
        result: Result<codex_app_server_protocol::PluginReadResponse, String>,
    ) {
        if let Err(err) = result {
            self.add_error_message(format!("Failed to load plugin detail: {err}"));
        }
    }
