...
- `collabToolCall` — `{id, tool, status, senderThreadId, receiverThreadIds, prompt?, agentsStates}` describing collab tool calls (`spawnAgent`, `sendInput`, `resumeAgent`, `wait`, `closeAgent`, `spawnTeam`, `waitTeam`, `closeTeam`); `status` is `inProgress`, `completed`, or `failed`.
