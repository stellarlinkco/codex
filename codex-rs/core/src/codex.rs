...
impl Codex {
    ...
    async fn spawn_internal(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        ...
        let user_instructions = get_user_instructions(
            &config,
            Some(loaded_skills.skills.as_slice()),
...
impl SessionConfiguration {
    ...
    pub(crate) fn apply(&self, updates: &SessionSettingsUpdate) -> ConstraintResult<Self> {
        ...
        let file_system_policy_matches_legacy = self.file_system_sandbox_policy
            == FileSystemSandboxPolicy::from(self.sandbox_policy.get());
...
impl Session {
    ...
    pub(crate) async fn execute_presentation_artifact(
        &self,
        request: codex_artifact_presentation::PresentationArtifactExecutionRequest,
        cwd: &std::path::Path,
    ) -> Result<
        codex_artifact_presentation::PresentationArtifactResponse,
        codex_artifact_presentation::PresentationArtifactError,
    > {
        let mut state = self.state.lock().await;
    ...
    pub(crate) async fn execute_spreadsheet_artifact(
        &self,
        request: codex_artifact_spreadsheet::SpreadsheetArtifactRequest,
        cwd: &std::path::Path,
    ) -> Result<
        codex_artifact_spreadsheet::SpreadsheetArtifactResponse,
        codex_artifact_spreadsheet::SpreadsheetArtifactError,
    > {
        let mut state = self.state.lock().await;
            ...
            match active.as_mut() {
                Some(at) => {
                    ...
                    ts.insert_pending_approval(
                        ...
                        grant_root
                            .as_ref()
                            .and_then(|root| AbsolutePathBuf::from_absolute_path(root.clone()).ok())
                            .into_iter()
                            .collect(),
                    )
                }
    ...
    pub async fn notify_request_permissions_response(
        ...
        match entry {
            Some(tx_response) => {
                tx_response.tx.send(response).ok();
            }
            None => {
        ...
        match entry {
            Some(tx_approve) => {
                if matches!(decision, ReviewDecision::ApprovedForSession) {
                    ...
                    store.approve_write_roots(tx_approve.approved_write_roots.clone());
                }
                tx_approve.tx.send(decision).ok();
            }
            None => {
    ...
    pub(crate) async fn transcript_path(&self) -> Option<PathBuf> {
        self.hook_transcript_path().await
    }
    ...
    pub(crate) async fn record_hook_context(
        ...
        let mut pending_hook_context = self.services.pending_hook_context.lock().await;
        pending_hook_context.extend(additional_context.iter().cloned());
    }
            ...
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    for hook_outcome in hook_outcomes {
                        ...
                        if let Some(error) = hook_outcome.result.error {
                            warn!(
                                turn_id = %turn_context.sub_id,
                                hook_name = %hook_name,
                                error = %error,
