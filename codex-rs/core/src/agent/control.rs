...
impl AgentControl {
    ...
    pub(crate) async fn spawn_agent_thread(
        ...
        let live_agent = self
            ...
            .await?;
        Ok((live_agent.thread_id, session_source))
    }
    ...
    pub(crate) async fn spawn_agent_thread_with_options(
        ...
        let live_agent = self
            .spawn_agent_with_metadata(config, Vec::new(), session_source.clone(), options)
            .await?;
        Ok((live_agent.thread_id, session_source))
    }
    ...
    pub(crate) async fn send_spawn_input(
        ...
        agent_id: ThreadId,
        items: Vec<UserInput>,
        _notification_source: Option<SessionSource>,
    ) -> CodexResult<String> {
    ...
    pub(crate) async fn shutdown_agent(&self, agent_id: ThreadId) -> CodexResult<String> {
        Box::pin(self.close_agent(agent_id)).await
    }
