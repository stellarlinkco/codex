...
async fn exec(
    ...
    if sandbox == SandboxType::WindowsRestrictedToken {
        let overlay = resolve_windows_restricted_token_filesystem_overlay(
            sandbox,
            sandbox_policy,
            file_system_sandbox_policy,
            network_sandbox_policy,
        ...
        .map_err(|reason| CodexErr::Io(io::Error::other(reason)))?;
        return exec_windows_sandbox(params, sandbox_policy, overlay.as_ref()).await;
    }
    let ExecParams {
        command,
        cwd,
...
pub(crate) fn resolve_windows_restricted_token_filesystem_overlay(
    ...
    {
        return Err(
            "windows sandbox backend cannot enforce unreadable split filesystem carveouts directly; refusing to run unsandboxed"
                .to_string(),
        );
    }
    ...
    if legacy_root_paths != split_root_paths {
        return Err(
            "windows sandbox backend cannot enforce split writable root sets directly; refusing to run unsandboxed"
                .to_string(),
        );
    }
    ...
    for writable_root in &split_writable_roots {
        for read_only_subpath in &writable_root.read_only_subpaths {
            if split_writable_roots.iter().any(|candidate| {
                ...
                return Err(
                    "windows sandbox backend cannot reopen writable descendants under read-only carveouts directly; refusing to run unsandboxed"
                        .to_string(),
                );
            }
        }
