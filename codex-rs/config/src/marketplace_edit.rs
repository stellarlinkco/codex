...
fn remove_marketplace(
    ...
    let (outcome, remove_marketplaces) = match marketplaces_item {
        TomlItem::Table(marketplaces) => {
            ...
            (
                outcome.clone(),
                outcome == RemoveMarketplaceConfigOutcome::Removed && marketplaces.is_empty(),
            )
        }
        ...
        _ => return RemoveMarketplaceConfigOutcome::NotFound,
    };
